...
class GPC(BaseEstimator,ClassifierMixin):
    ...
    def predict(self,X):
        ...
        K_a = self.covfn(self.X_train,self.X_train,self.hyper) + self.sima_n * np.eye(self.X_train.shape[0])
...
from sklearn.gaussian_process import GaussianProcessClassifier as GPC2
from sklearn.gaussian_process.kernels import ConstantKernel, RBF

print('GPC() code verification only ...')
...
# GPC Model
model = GPC([1.0,1.0],opt=True,verbose=True,return_std=True).fit(X,y)
yp = model.predict(X)
yp_b = model.predict_proba(X) 
print(yp_b.shape)

# GPC SKL Model
rbf = ConstantKernel(1.0) * RBF(length_scale=1.0)
gpc = GPC2(kernel=ConstantKernel(1.0) * RBF(length_scale=1.0))
gpc.fit(X,y.ravel())
yp2 = gpc.predict(X)
yp2_b = gpc.predict_proba(X) 
print(yp2_b.shape)

sklearn_theta_0 = gpc.kernel_.k2.get_params()['length_scale']
sklearn_theta_1 = np.sqrt(gpc.kernel_.k1.get_params()['constant_value'])
print(sklearn_theta_0,sklearn_theta_1,-gpc.log_marginal_likelihood_value_)
print(model.hyper,model.nll)
...
clf = GPC([1.0,1.0],opt=True,verbose=False,return_std=False)
# rbf = ConstantKernel(1.0) * RBF(length_scale=1.0)
clf2 = GPC2(kernel=ConstantKernel(1.0)*RBF(length_scale=1.0))
