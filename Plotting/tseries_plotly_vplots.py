...
# Subplot Plotly Timeseries Line Plots (Plot n verticle subplots)
# Input x axis -> date_time # lst-> column plots, # nplots -> total number of subplots,
#       lw_id -> lineplot thickness list

def plot_vsubplots(ldf=None,lst=None,title=None,nplots=None,lw_id=None,size=[500,1000]):
    ...
    fig.update_layout(height=size[0],width=size[1],template='plotly_white',title=title,
                      ...
                      font=dict(family='sans-serif',size=12));fig.show()
...
# plot example 
plot_vsubplots(df,['A','B'],title='<b>SUBPLOTS</b> | verticle plots',nplots=2,lw_id=[2,2])
